...
class SleepState:
    ...
    def draw(boy):
        ...
        #Boy.image.clip_composite_draw(0, )
next_state_table = {
...
class Boy:
    ...
    def __init__(self):
        ...
        if Boy.line_image == None:
            Boy.line_image = load_image('../res/Line.png')
    ...
    def draw(self):
        ...
        #Boy.line_image.clip_composite_draw(0, 0, 134, 43, math.atan2(Boy.MouseY - self.y, Boy.MouseX - self.x), '', self.x, self.y, math.sqrt((Boy.MouseX - self.x) ** 2 + (Boy.MouseY - self.y) ** 2), 2)
    ...
    def handle_event(self, e):
        ...
        if e.type == SDL_MOUSEMOTION:
            Boy.MouseX, Boy.MouseY = e.x, 600 - e.y
    ...
    def fire_ball(self, big):
        ...
        ballSpeed = abs(Boy.MouseX - self.x) / 25 if self.dir == 1 else -1 * abs(Boy.MouseX - self.x) / 25
        ySpeed = abs(Boy.MouseY - self.y) / 25 if self.dir == 1 else -1 * abs(Boy.MouseY - self.y) / 25
        if big: ySpeed *= 0.75
